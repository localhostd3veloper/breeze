...
Keeping them apart is what lets web search and generative UI happen in the *same*
answer. A tool-role message ties the answer to a tool-calling model, and
`gemma3:12b` cannot call tools at all -- which is why search used to veto widgets.

### Model selection

The answer model is picked *after* acquire, so it follows what actually happened.
...
<Callout title="Evidence, not the toggle">
  Search is on by default, so keying the model off the *flag* would put every
  "hi" on the tool-capable model. It keys off `has_evidence` instead -- a turn
  that needed no search is still answered by the small default model.
</Callout>
...
- **Images pin the model and the client.** A remote generative-UI endpoint may not
  ...
  widget model. The widget *grammar* still rides along, so "chart what is in this
  screenshot" works.
