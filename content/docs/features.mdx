...
<Callout title="Stacking is allowed; the model is still one model">
  ...
  **images > thinking > web search > default** -- so the highest-priority active
  mode picks the model. See [model selection](/docs/architecture#model-selection).
</Callout>
...
<Callout type="warn">
  This is the **only** feature that leaves your machine, and it is off until you
  switch it on. It also needs `TAVILY_API_KEY` set in `backend/.env` -- without
  it the toggle is accepted and silently does nothing.
</Callout>

The full two-pass tool flow is in the [Web search guide](/docs/web-search).
...
Attach an image and Breeze switches to a vision model (`gemma3:12b`). You can
attach several to one turn.
