...
const CVD_TARGET = 8.0,
  CVD_FLOOR = 6.0; // OKLab Delta E×100, min(protan, deutan), adjacent pairs
...
export function validateOrdinal(palette, { mode = 'light', surface } = {}) {
  ...
  const thin = gaps
    .map((g, i) => [palette[i], palette[i + 1], g])
    .filter(([, , g]) => g < ORDINAL_MIN_DL)
    .map(([a, b, g]) => [a, b, +g.toFixed(3)]);
...
if (typeof document !== 'undefined') {
  ...
  if (b?.dataset.palette) {
    ...
    const badEnum = !['light', 'dark'].includes(mode)
      ? `data-mode ${JSON.stringify(mode)}`
      : !['adjacent', 'all'].includes(pairs)
    ...
    if (!palette.length || badEnum || badHex.length) {
      ...
      console.warn(
        `validate_palette: ${!palette.length ? 'empty palette' : badEnum ? `unrecognized ${badEnum}` : `invalid hex value(s): ${badHex.join(', ')} \u2014 expected #rrggbb`} \u2014 not validating`
      );
    } else {
